...
use crate::{AniMeDataBuffer, AniMeGif, AniMeImage, error::AnimeError};
...
impl Sequences {
    ...
    pub fn add_asus_gif(&mut self, file: &Path,duration: Option<Duration>, brightness: f32) -> Result<(), AnimeError> {
    ...
    pub fn add_png(
        ...
        let image = AniMeImage::from_png(file, scale, angle, translation, duration, brightness)?;
    ...
    pub fn add_image_gif(
        ...
        let frames = AniMeGif::create_png_gif(file, scale, angle, translation, duration, brightness)?;
        self.0.push(Action::Animation(frames));
        Ok(())
    }
